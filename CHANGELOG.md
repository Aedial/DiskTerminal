...
- Add Compacting Storage Cells that expose compressed/decompressed item forms to the ME network
  ...
  - If partitioned from the Cell Terminal, the chain is automatically initialized. Inserting the partitioned item is not required.
...
- Add Storage Cells for larger capacities:
  ...
  - High-Density Storage Cells: 1k to 1G (multiplying base size by 2.1B)
- Add priority field to set ME Chest/Drive priority directly from the Terminal GUI
- Add Priority Wand item to set ME Chest/Drive priority by right-clicking them
