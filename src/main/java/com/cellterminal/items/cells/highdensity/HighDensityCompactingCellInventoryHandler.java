...
/**
 * Inventory handler wrapper for high-density compacting cells.
 * Handles partition filtering and upgrade processing.
 */
public class HighDensityCompactingCellInventoryHandler extends MEInventoryHandler<IAEItemStack> implements ICellInventoryHandler<IAEItemStack> {

    private IncludeExclude myWhitelist = IncludeExclude.WHITELIST;

    public HighDensityCompactingCellInventoryHandler(IMEInventory<IAEItemStack> inventory, IStorageChannel<IAEItemStack> channel) {
    ...
    public boolean passesBlackOrWhitelist(IAEItemStack input) {
        ...
        ICellInventory<IAEItemStack> ci = getCellInv();
        if (!(ci instanceof HighDensityCompactingCellInventory)) return false;

        HighDensityCompactingCellInventory hdCompacting = (HighDensityCompactingCellInventory) ci;
