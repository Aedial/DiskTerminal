...
/**
 * High-Density storage cell item.
 * 
 ...
 */
public class ItemHighDensityCell extends ItemHighDensityCellBase {
    ...
    public ItemHighDensityCell() {
        super(TIER_NAMES, DISPLAY_BYTES, DISPLAY_BYTES_PER_TYPE);
        setRegistryName(Tags.MODID, "high_density_cell");
        setTranslationKey(Tags.MODID + ".high_density_cell");
    }

    @Override
    protected ItemStack getCellComponent(int tier) {
        // High-density cells use high-density components
        return ItemHighDensityComponent.create(tier);
    }

    /**
     * Create a cell ItemStack for the given tier.
     * @param tier 0=1k, 1=4k, 2=16k, 3=64k, 4=256k, 5=1M, 6=4M, 7=16M, 8=64M, 9=256M, 10=1G
     */
    public static ItemStack create(int tier) {
        if (tier < 0 || tier >= TIER_NAMES.length) tier = 0;

        return new ItemStack(com.cellterminal.ItemRegistry.HIGH_DENSITY_CELL, 1, tier);
    }
    ...
    /**
     * Get the number of tiers available.
     */
