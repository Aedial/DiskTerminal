...
/**
 * Manages priority text fields for visible storage entries.
 * Each storage gets its own text field positioned at the far right of its header line.
 */
public class PriorityFieldManager {
     ...
     * @param x Unused (kept for API compatibility)
     ...
     */
    public void updateFieldPosition(PriorityField field, int x, int y, int guiLeft, int guiTop) {
    ...
    public boolean handleClick(int mouseX, int mouseY, int mouseButton) {
        // First check if clicking on any visible field
        for (PriorityField field : fields.values()) {
    ...
    public boolean handleKeyTyped(char typedChar, int keyCode) {
        ...
        return focusedField.keyTyped(typedChar, keyCode);
    }
    ...
    public static class PriorityField {
        ...
        public boolean keyTyped(char typedChar, int keyCode) {
            // Enter submits
            if (keyCode == 28 || keyCode == 156) { // KEY_RETURN or KEY_NUMPADENTER
                submitPriority();
            ...
            if (Character.isDigit(typedChar) || typedChar == '-'
                    || keyCode == 14 || keyCode == 211  // BACKSPACE, DELETE
                    || keyCode == 203 || keyCode == 205  // LEFT, RIGHT
                    || keyCode == 199 || keyCode == 207) { // HOME, END
