...
/**
 * Packet sent from server to client when priority is successfully applied via wand.
        ...
        public IMessage onMessage(PacketPriorityApplied message, MessageContext ctx) {
            Minecraft.getMinecraft().addScheduledTask(() -> {
                ...
                String msg = I18n.format(
                    "cellterminal.priority.applied",
                    message.priority,
                    message.pos.getX(), message.pos.getY(), message.pos.getZ()
                );
                Minecraft.getMinecraft().player.sendMessage(
                    new TextComponentString(TextFormatting.GREEN + msg)
                );
            });
